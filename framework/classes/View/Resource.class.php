...
class Resource extends \CADB\Objects {
	...
	public static function addCss($css,$priority=0,$options='') {
		if(preg_match("/(.+)\.js$/i",$css)) {
			...
			self::pushCss(CADB_RESOURCE_URI.'/css/'.$css,$priority,'src',$options);
		}
	}
	...
	public static function addJsCss($css,$priority=0,$options='') {
		if(preg_match("/(.+)\.js$/i",$css)) {
			...
			self::pushCss(CADB_RESOURCE_URI.'/js/'.$css,$priority,'src',$options);
		}
	}
	...
	public static function addLibraryCss($css,$priority=0,$options='') {
		if(preg_match("/(.+)\.js$/i",$css)) {
			...
			self::pushCss(CADB_CONTRIBUTE_URI."/".$css,$priority,'src',$options);
		}
	}
	...
	public static function addScript($js,$priority=0,$options='') {
		if(preg_match("/(.+)\.css$/i",$js)) {
			...
			self::pushJs(CADB_RESOURCE_URI.'/script/'.$js,$priority,'src',$options);
		}
	}
	...
	public static function addLibraryJs($js,$priority=0,$options='') {
		if(preg_match("/(.+)\.css$/i",$js)) {
			...
			self::pushJs(CADB_CONTRIBUTE_URI.'/'.$js,$priority,'src',$options);
		}
	}
