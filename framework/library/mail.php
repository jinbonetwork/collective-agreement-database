...
function sendEmail($senderName, $senderEmail, $receivers, $subject, $message, $logging=0) {
	require_once( CADB_CONTRIBUTE_PATH."/PHPMailer/class.smtp.php" );
	require_once( CADB_CONTRIBUTE_PATH."/PHPMailer/class.phpmailer.php" );

	$context = \CADB\Model\Context::instance();
	$service = $context->getProperty('service.*');

	$mail = new PHPMailer();
	$mail->SetLanguage( 'en', CADB_CONTRIBUTE_PATH."/PHPMailer/language/" );
	$mail->IsHTML(true);
	if($service['useSMTP']) {
		$mail->IsSMTP();
...
function makeLetter($variables) {
	...
	ob_start();
	include_once CADB_RESOURCE_PATH."/html/letter.html.php";
